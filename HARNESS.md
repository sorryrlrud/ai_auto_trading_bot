...
- Candidate entries are hard-blocked when the long-term trend is not aligned, the 1-hour or 15-minute trend is not aligned, the daily Bollinger position exceeds `MAX_ENTRY_BB_POSITION` (default `1.05`), the market is overheated, the move is too extended, or `atr_pct` exceeds `MAX_ENTRY_ATR_PCT` (default `6.0`).
